...
void update_pos(int x,int pos,int val,int L,int R) {
    x=(L+R)|(L<R);
    if (L==R) {
        pval[x]=merge(pval[x],make_pair(val,-INF));
        return;
    } int mid=(L+R)/2;
    if (pos<=mid) update_pos(x<<1,pos,val,L,mid);
    if (mid<pos) update_pos(x<<1|1,pos,val,mid+1,R);
    pval[x]=merge(pval[x<<1],pval[x<<1|1]);
}
pii query_pos(int x,int l,int r,int L,int R) {
    x=(L+R)|(L<R);
    // printf("%d %d %d %d %d\n",x,l,r,L,R);
    if (l<=L&&R<=r) return pval[x];
    int mid=(L+R)/2; pii ret(-INF,-INF);
    if (l<=mid) ret=merge(ret,query_pos(x<<1,l,r,L,mid));
    if (mid<r) ret=merge(ret,query_pos(x<<1|1,l,r,mid+1,R));
    return ret;
}
int T[maxn*4];
pii lp[maxn],rp[maxn];
...
int main() {
    ...
    ll ans=0; int k=0;
    FOR(i,1,n) {
        if (lp[i].second) update(1,lp[i].first,lp[i].second,1,n);
        while (k<(int)queries.size()&&queries[k].first==i) {
            auto now=queries[k++];
            ans+=now.second.second*query(1,now.second.first,1,n);
        }
    } printf("%lld\n",ans);
}
/*
