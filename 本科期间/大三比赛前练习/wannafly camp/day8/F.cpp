...
int main() {
	...
	scanf("%d",&n);
	REP(i,n) scanf("%lld",&w[i]),w[i]=w[i]*fac[i]%M;

	// int t,j;
	// REP(t,n) REP(i,n) REP(j,n) if (i-t>=0&&j-(n-t-1)>=0){
	// 	add_(W[i+j],w[i]*fac[i]*inv[i-t]*w[j]*fac[j]*inv[j-(n-t-1)]%M);
	// }

	// int k,j;
	// REP(i,n) REP(j,n) REP(k,n) {
	// 	if (j-(n-(i-k)-1)>=0){
	// 		add_(W[i+j],w[i]*w[j]*inv[i+j-k-(n-1)]*inv[k]%M);
	// 	}
	// }
	// REP(i,n*2) printf("%lld ",W[i]); puts("<- w");

	NTT::multiply(w,w,_w,n,n);

	REP(i,n) w[i]=inv[i];
	NTT::multiply(w,w,W,n,n);
	REP(i,n) _w[i]=_w[i+n-1]*W[i]%M;

	// REP(i,n+1) iw[i]=inv[n-i];
	// NTT::multiply(w,iw,_w,n+1,n+1);

	// REP(i,n*2) printf("%lld ",w[i]); puts("<- w");
	// REP(i,n*2) printf("%lld ",_w[i]); puts(" <- _w");
	// REP(i,n*2) _w[i]=W[i+n-1]%M;
	// REP(i,n*2) printf("%lld ",W[i]); puts("");
	REP(i,n) printf("%lld ",_w[i]);
}
/*
4
1 1 1 1
