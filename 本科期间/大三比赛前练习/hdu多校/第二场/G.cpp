...
int lazy[maxn*4],MIN[maxn*4],SUM[maxn*4];
int A[maxn],B[maxn];
...
void update(int x,int l,int r,int L,int R){
	if (l<=L&&R<=r){
		if (MIN[x]>1) change(x,1);
		else{
			if (L==R) {
				A[L]+=lazy[x]+1; lazy[x]=0;
				SUM[x]=A[L]/B[L]; MIN[x]=B[L]-A[L]%B[L];
				// printf("change: %d; A=%d; B=%d\n",L,A[L],B[L]);
				return;
			}int mid=(L+R)/2;
			pushdown(x);
