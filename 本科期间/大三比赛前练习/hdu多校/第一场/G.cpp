...
int TaskA() {
	...
	rREP(i,62){
		if (n>=cnt[i]){
			add_(ans,pre[i]); n-=cnt[i];
			if (n) add_(ans,n%M*((1ll<<i)%M)%M);
		} if (n>=cnt[i]){
			add_(ans,pre[i]); n-=cnt[i];
		}
	} printf("%d\n",ans);
...
void TaskB(){
	...
	A[1]=A[2]=1; int i,n=64;
	FOR(i,3,n) A[i]=A[i-A[i-1]]+A[i-1-A[i-2]];
	FOR(i,1,n) cnt[A[i]]++;
	FOR(i,2,n) A[i]+=A[i-1];
	FOR(i,1,n) printf("%d ",A[i]-1); puts("");
	// FOR(i,1,n) if (cnt[i]) printf("%d: %d\n",i,cnt[i]);
}
void initialize() {
	// TaskB();
	int i; pre[0]=1; cnt[0]=1;
	rep(i,1,64) pre[i]=(pre[i-1]*2+(1ll<<i)%M+(1ll<<(i-1))%M*(cnt[i-1]%M)%M)%M,cnt[i]=cnt[i-1]*2+1;
	// rep(i,1,64) printf("%d : %lld %lld\n",i,pre[i],cnt[i]);
}
int main() {
	int startTime=clock();
	//initialize
