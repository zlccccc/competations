...
void Update(int &x,int l,int r,int val,int L,int R){
	if (!x) x=++cnt;
	if (l<=L&&R<=r){T[x].val.push_back(val);return;}
	int mid=(L+R)/2;
...
int Query(int &x,int Y,int L,int R){
	if (!x) return 0;
	while (T[x].val.size()&&mark[T[x].val.back()]) T[x].val.pop_back();
	int ret=0,t=0;
	if (T[x].val.size()) ret=T[x].val.back();
	int mid=(L+R)/2;
	if (mid>=Y) t=Query(T[x].l,Y,L,mid);
	else t=Query(T[x].r,Y,mid+1,R);
	if (t) return t;
	return ret;
}
int query(int x,int X,int Y,int L,int R){
	int ret=Query(root[x],Y,1,m),t=0;
	if (L==R) return ret;
	int mid=(L+R)/2;
	if (mid>=X) t=query(x<<1,X,Y,L,mid);
	else t=query(x<<1|1,X,Y,mid+1,R);
	if (t) return t;
	return ret;
}
int i,j,k;
...
int main(){
	...
	FOR(i,1,q){
		...
		if (k==1){
			...
			update(1,now.x1,now.x2,now.y1,now.y2,i,1,n);
			MP[now]=i;
		}else if (k==2){
			scanf("%d%d%d%d",&now.x1,&now.y1,&now.x2,&now.y2);
			mark[MP[now]]=1;
