...
int main() {
    ...
    FOR(i,1,m) FOR(j,i+1,m) {
        if (SegmentProperIntersection(P[u[i]],P[v[i]],P[u[j]],P[v[j]])) {
            ...
            // P[v[j]].print();
            // puts("");
        } else if (OnSegment(P[u[i]],P[u[j]],P[v[j]])||
                    OnSegment(P[v[i]],P[u[j]],P[v[j]])||
                    OnSegment(P[u[j]],P[u[i]],P[v[i]])||
                    OnSegment(P[v[j]],P[u[i]],P[v[i]])) {
            if (u[i]==u[j]||u[i]==v[j]) continue;
            if (v[i]==u[j]||v[i]==v[j]) continue;
            ans++;
            // printf("%d-%d: (2\n",i,j);
            // printf("%d %d %d %d\n",OnSegment(u[i],u[j],v[j]),OnSegment(v[i],u[j],v[j]),
            //                        OnSegment(u[j],u[i],v[i]),OnSegment(v[j],u[i],v[i]));
            // P[u[i]].print();
            // P[v[i]].print();
            // P[u[j]].print();
            // P[v[j]].print();
