...
int A[maxn],A_B[maxn],B_A[maxn];
ll B[maxn],ans[maxn];
int main() {
	int i,p,j;
	scanf("%d%d",&n,&p);
	FOR(i,1,n) scanf("%d",&A[i]);
	int g; rFOR(g,2,p-1) if (isroot(g,p)) break;
	j=1; int cnt_0=0;
	FOR(i,0,p-2) A_B[j]=i,B_A[i]=j,j=(ll)j*g%p;
	FOR(i,1,n) if (A[i]%p==0) cnt_0++; else B[A_B[A[i]%p]]++;
	ans[0]=(ll)cnt_0*(n-cnt_0)*2+(ll)cnt_0*cnt_0;
	FFT::mul(B,B,B,2*p,2*p);
	FOR(i,0,p+p) ans[B_A[i%(p-1)]]+=B[i];
	FOR(i,1,n) if (A[i]<=p) printf("%lld\n",ans[A[i]]); else puts("0");
}
/*
*/
