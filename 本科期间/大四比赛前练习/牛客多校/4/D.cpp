...
ll getvalue(ll n,int k,int x0,int x1){
	ll i;
	if (k) {
		REP(i,63) if ((n>>i)&1) {
			if (!x0&&!x1) break;
			if (i%2==0&&x0) x0--,n^=1ll<<i;
			else if (i%2==1&&x1) x1--,n^=1ll<<i;
		} 
		if (!x0&&!x1) return n;
		return 0;
	} else {
...
int solve2(ll n) {
	...
	REP(i,3) REP(j,3) REP(k,3) REP(l,3) if ((i*1+j*2)%3==all&&(k*1+l*2)%3==all){
		ll A=getvalue(n,0,i,j),B=getvalue(n,1,k,l);
		// printf("%lld %d %d %d\n %lld %lld; %lld\n",i,j,k,l,A,B,A|B);
		if ((A|B)==n) return 0*printf("2 %lld %lld \n",A,B);
	}
	return 0;
}
int main() {
