...
int base[107][107];
// int base[107][107][107][107];
int below[107][107];//(a,b) <- (x,y)
int pw[107];//pow(x,n)
void init() {
	int i,j,x,y,u,v;
	fac[0]=1;
	...
	FOR(i,0,31) {//base
		base[i][i]=1;
		FOR(x,0,i) if (x!=i) { //base
			ll bbase=C(i,x);
			FOR(u,0,x) {
				add_(base[i][u],M-bbase*base[x][u]%M);
			}
		}
	}
...
int main() {
	...
	FOR(_,1,T) {
		...
		// }
		FOR(i,0,c0) FOR(j,0,c1) if (base[c0][i]&&base[c1][j]&&below[i][j]){
			add_(ans,(ll)base[c0][i]*base[c1][j]%M*powMM(below[i][j],n)%M);
		}
		// FOR(i,0,c0) {
		// 	FOR(j,0,c1) printf("%lld ",powMM(M-1,i+j+1)*(ll)base[c0][i]%M*base[c1][j]%M); puts("");
		// }
		printf("%d\n",ans);
	}
}
