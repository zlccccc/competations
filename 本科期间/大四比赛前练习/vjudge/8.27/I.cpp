...
char str[107][107];
int mark[107*107];
void dfs(int x,int id) {//linked; 已经cut; id=1:T
	printf("dfs: %d (%d %d)\n",x,(x-1)/m+1,(x-1)%m+1);
	mark[x]=id;
	for (int i=maxflow::head[x]; ~i; i=maxflow::edge[i].next) {
		if (!maxflow::edge[i^id].cap) continue;
		int v=maxflow::edge[i].to;
		if (mark[v]==-1) dfs(v,id);
	}
}
int S,T;
...
int main() {
	...
	FOR(_,1,_caseT) {
		...
		FOR(i,1,T) mark[i]=-1;//marked; must
		dfs(S,0); dfs(T,1);
		FOR(i,1,n) FOR(j,1,m) {
			if (str[i][j]!='.') continue;
			...
			if (base!=-1) {
				base^=ini[i][j];
				if (!base) {
					if (mark[getid(i,j)]!=1) {//linkS
						str[i][j]='B';
						maxflow::addedge(S,getid(i,j),INF);
						dfs(S,0); 
					} else {//linkT
						str[i][j]='W';
						maxflow::addedge(getid(i,j),T,INF);
						dfs(T,1); 
					}
				} else {
					if (mark[getid(i,j)]!=0) {
