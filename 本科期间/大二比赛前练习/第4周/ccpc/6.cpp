...
char val[maxn];
node sum[maxn<<2],rev[maxn<<2];
bool flip[maxn<<2];
void pushdown(int x){
	if (flip[x]){
		flip[x<<1]^=1;
		flip[x<<1|1]^=1;
		swap(sum[x<<1],rev[x<<1]);
		swap(sum[x<<1|1],rev[x<<1|1]);
		flip[x]=0;
	}
}
void pushup(int x){
	sum[x]=sum[x<<1]*sum[x<<1|1];
	rev[x]=rev[x<<1]*rev[x<<1|1];
}
void build(int x,int l,int r){
	flip[x]=0;
	if (l==r){
		if (val[l]=='1') sum[x]=mul[1],rev[x]=mul[0];
		if (val[l]=='0') sum[x]=mul[0],rev[x]=mul[1];
		return;
	}
	int mid=(l+r)/2;
	build(x<<1,l,mid);
