...
void calc(int x){
	...
	rREP(i,32){
		int mark=((x&(1<<i))!=0);
		if (mark==1&&nxt[now][1]) ans+=num01[nxt[now][1]];
		if (mark==0&&nxt[now][0]) ans+=num10[nxt[now][0]];
...
//			printf("%d ",ans);
		...
		printf("%lld\n",ans);
		FOR(i,0,cnt) num[i]=num01[i]=num10[i]=nxt[i][0]=nxt[i][1]=0;
	}
}
/*
8
