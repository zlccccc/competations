...
int solve(){
	...
	while (tot<m){
		...
		if (tot>m) break;
		now++;
		if (pos[tot%3]==V[tot%3].size()&&tot%3==0) return 0*puts("-1");
		else if (pos[tot%3]==V[tot%3].size()){
			if (pos[3-tot%3]==V[3-tot%3].size()) return 0*puts("-1");
			ans[now][0]=V[3-tot%3][pos[3-tot%3]].second;
			ans[now][1]=V[3-tot%3][pos[3-tot%3]].first;
			ans[now][2]=tot;
			pos[3-tot%3]++;
		}else{
