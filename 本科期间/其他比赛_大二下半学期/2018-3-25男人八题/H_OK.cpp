...
//    printf("change_L: %lld %d;   sum=%lld,   sum_sum=%lld\n",val,seg,sum,sum_sum);
    cnt+=seg;sum+=val*seg;
}
void changer(LL val,int seg){
    sum_sum-=sum*seg*2;
    sum_sum+=cnt*val*seg*2;
//    printf("change_R: %lld %d;   sum=%lld,   sum_sum=%lld\n",val,seg,sum,sum_sum);
    cnt+=seg;sum+=val*seg;
}
int _l,_r;
LL A[maxn];
...
//                LL rsum=(n-m+1)*(getR(i+1,m)+i*(A[m]-A[i]));
//                printf("%d -> %d : M=%lld; R=%lld;  pre=%lld; ans=%lld\n",i,m,msum,rsum,pre[i],pre[i]-msum-rsum);
////                printf("%d -> %d : %lld; L=%lld;",i,m,sum_sum,L);
////                printf("open:  %lld   %lld %lld\n",i*getL(i,m-1),(n-m+1)*getR(i+1,m),(A[m]-A[i]));
