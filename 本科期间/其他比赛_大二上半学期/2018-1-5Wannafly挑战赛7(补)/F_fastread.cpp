...
int main(){
    RI(n);RI(m);
    REP(i,n) RI(a[i].pos),a[i].type=0;
    REP(i,m) RI(a[i+n].pos),RI(a[i+n].type);
    sort(a,a+n+m,cmp);
    REP(i,n+m){
...
//    REP(i,n+m) printf("%-12d%-12d:  %d %d\n",a[i].pos,a[i].type,K[i],H[i]);puts("");
//    REP(i,N) printf("%d(%d) ",A[i].pos,A[i].type);puts("");
    now=MID;
    ...
    REP(i,N){
        if (!A[i].type){//mouse
            ...
            L-=A[i].pos;R+=A[i].pos;
        }assert(MID-(N-n)<=now&&now<=MID+(N-n));
//        printf("now,F= %d %lld; L,R=%lld %lld\n",now-MID,F[now],L,R);
    }printf("%lld\n",F[now]);
}
/*
*/
