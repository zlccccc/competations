...
int id[maxn],revid[maxn],dep[maxn];
int tot;
stack<int> STACK;
int fa[maxn][25];
void dfs(){
	int i;
	STACK.push(1);dep[1]=1;
	while (STACK.size()){
		int top=STACK.top();STACK.pop();
		id[top]=++tot;revid[tot]=top;
		for (int v:edge[top]) if (!id[v]){
			STACK.push(v);
			dep[v]=dep[top]+1;
			fa[v][0]=top;
			rep(i,1,25) fa[v][i]=fa[fa[v][i-1]][i-1];
		}
	}
}
int lca(int x,int y){
	...
	rREP(i,25) if (dep[x]-dep[y]>=1<<i) x=fa[x][i];
	if (x==y) return x;
	rREP(i,25) if (fa[x][i]!=fa[y][i]) x=fa[x][i],y=fa[y][i];
	return fa[x][0];
}
int lca_k[maxn];
set<int> pos[maxn];  
