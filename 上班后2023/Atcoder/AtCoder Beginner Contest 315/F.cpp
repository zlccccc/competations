...
int main() {
    ...
    FOR(_,1,T){
        int n;
        scanf("%d",&n);
        FOR_(i,1,n) scanf("%d%d",&X[i],&Y[i]);
        FOR_(i,2,n) {
            FOR_(x,0,30) {
                f[i][x]=1e18;
                FOR_(j,max(1,i-x-1),i-1) {
                    double dis=sqrt((X[i]-X[j])*(X[i]-X[j])+(Y[i]-Y[j])*(Y[i]-Y[j]));
                    int y=i-j-1;
                    // printf("%d->%d %d->%d :%lf\n",j,i,x-y,x,f[j][x-y]+dis);
                    f[i][x]=min(f[i][x],f[j][x-y]+dis);
                }
            }
        }
        FOR_(x,1,30) f[n][x]+=pow(2,x-1);
        double ans=1e18;
        FOR_(x,0,30) ans=min(ans,f[n][x]);
        printf("%.20lf\n",ans);
    }
}
/*
