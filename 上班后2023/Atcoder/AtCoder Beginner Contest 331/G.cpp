...
int main() {
    ...
    FOR(i,1,1000000) inv[i]=inv[i]*inv[i-1]%MOD;// inv(n!)
    scanf("%d%d",&n,&m);
    FOR(i,1,m) {
        int x;
        scanf("%d",&x);
        poly res; res.resize(x+1);
        FOR(j,1,x) res.v[j]=C(x,j);
        vec[x].push(res);
    }
    poly res;
    FOR(i,1,n+m) {
        while (vec[i].size()) {
            auto cur=vec[i].front(); vec[i].pop();
            if (res.size()==0) res=cur;
            else {
                res=res*cur;
                vec[res.size()-1].push(res);
                res=poly();
            }
            // printf("cur.size=%d -> %d\n",cur.size()-1,res.size()-1);
        }
    }
    // for (int v:res.v) printf("%d ",v); puts("<- res");
    ll ans=0;
    REP(i,n) {
        ll prob=(MOD+1-res[i]*inver(C(n,i))%MOD)%MOD;  // 选择了i个的时候没有选够; 要选第i+1张卡
        ll weight=(n*inver(n-i))%MOD;  //inver(C(n,i))
        ans=(ans+weight*prob)%MOD;
    }
    printf("%lld\n",ans);
}
/*
