...
void ADD(int v,int c) {
    nowans-=(ll)cnt[v]*(cnt[v]-1)*(cnt[v]-2)/6;
    cnt[v]+=c;
    nowans+=(ll)cnt[v]*(cnt[v]-1)*(cnt[v]-2)/6;
}
ll ans[maxn];
int main() {
    ...
    FOR_(i,1,q) scanf("%d%d",&Q[i].l,&Q[i].r),Q[i].id=i;
    FOR_(i,1,n) BLOCK[i]=i/SIZE;
    sort(Q+1,Q+1+q,[&](query &x,query &y){
