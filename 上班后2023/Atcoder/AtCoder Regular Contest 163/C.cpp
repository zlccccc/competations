...
vector<int> solve(int x,int y) {
    int now=1;
    vector<int> ret;
    for (int i=1;now*3<=x&&i<y;i++) now*=3,ret.push_back(now);
    ret.push_back(now*2);
    return ret;
}
int main() {
    ...
    FOR(_,1,T){
        ...
        if (n==1) {
            ...
            vector<int> ans={2,3,6}; n-=3;
            while (n) {
                set<int> now;
                reverse(ans.begin(),ans.end());
                for (int v:ans) {
                    if ((ll)v*(v+1)<=1e9&&n&&!now.count(v+1)&&!now.count(v*(v+1))) {
                        now.insert(v*(v+1));
                        now.insert(v+1);
                        // now.insert(v*6);
                        n--;
                ...
                // printf("ans.size=%d\n",now.size());
                ans.clear();
                // for (int v:now) printf("%d ",v); puts("<- v");
                for (int v:now) ans.push_back(v);
            }
            puts("Yes");
