...
// sum{ai+b/c}, 0->n
mint f(LL a,LL b,LL c,LL n) {
    if (a==0) return b/c*(n+1);
    if (a>=c||b>=c) return mint(a/c)*n*(n+1)/2+mint(b/c)*(n+1)+f(a%c,b%c,c,n);
    ll m=(a*n+b)/c;
    return mint(n)*mint(m)-f(c,c-b-1,a,m-1);
}
ll A[maxn];
int main() {
    int n;
