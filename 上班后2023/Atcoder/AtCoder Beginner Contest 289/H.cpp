...
int main() {
    ...
    invpw2[0]=1;
    FOR(i,1,1000000) invpw2[i]=invpw2[i-1]*inv[2]%M;

    int T;
    scanf("%d%d%d%d",&val[0],&val[1],&val[2],&T);
    sort(val,val+3); val[1]-=val[0]; val[2]-=val[0]; val[0]=0; val[1]/=2; val[2]/=2;
    ...
    // calculate f and ff
    {
        ...
        polynomial::poly C=A*A;
        FOR(i,0,T) f[i]=C.v[i]*fac[i]%M*fac[i]%M*fac[i]%M*invpw2[i]%M*invpw2[i]%M*invpw2[i]%M;
    }
    {
        polynomial::poly A,B;
        A.init(T+1,[&](int x) {
            return inv[x]%M*inv[x+val[1]]%M*inv[x+val[2]]%M;;
        });
        B.init(T+1,[&](int x) {
            if (x<val[2]) return 0ll;
            return inv[x]%M*inv[x-val[1]]%M*inv[x-val[2]]%M;
        });
        // FOR(i,0,T-val[2]) A.v[i]=
        polynomial::poly C=A*B;
        FOR(i,val[2],T) ff[i]=C.v[i]*fac[i]%M*fac[i]%M*fac[i]%M*invpw2[i]%M*invpw2[i]%M*invpw2[i]%M;
    }
    {
        polynomial::poly F;
        F.resize(T+1);
        ...
        polynomial::poly GG=FF-FF*(G-1);
        FOR(i,0,T) gg[i]=GG.v[i];
    }

    // // FOR(i,1,T) printf("%lld ",f[i]); puts("<- f");
    // FOR(i,1,T) printf("%lld ",g[i]); puts("<- g");
    // FOR(i,1,T) printf("%lld ",gg[i]); puts("<- gg");
    // // FOR(i,1,T) printf("%lld ",ff[i]); puts("<- ff");
    // // f[0]=1;
    // // FOR(i,1,T) {
    // //     // f[i]=0;
    // //     // FOR(j,0,i) add_(f[i],C(i,j)*C(i,j)%M*C(i,j)%M*invpw2[i]%M*invpw2[i]%M*invpw2[i]%M);
    // //     g[i]=f[i];
    // //     FOR(j,1,i-1) add_(g[i],M-f[j]*g[i-j]%M);
    // // }
    // FOR(i,val[2],T) {
    //     // ff[i]=0;
    //     // FOR(j,0,i-val[2]) add_(ff[i],C(i,j)*C(i,j+val[1])%M*C(i,j+val[2])%M*invpw2[i]%M*invpw2[i]%M*invpw2[i]%M);
    //     gg[i]=ff[i];
    //     FOR(j,1,i-1) add_(gg[i],M-ff[j]*g[i-j]%M);
    //     // add_(ans,gg[i]);
    // }

    // FOR(i,1,T) printf("%lld ",f[i]); puts("<- real f");
    // FOR(i,1,T) printf("%lld ",g[i]); puts("<- real g");
    // FOR(i,1,T) printf("%lld ",ff[i]); puts("<- real ff");
    // FOR(i,1,T) printf("%lld ",gg[i]); puts("<- real gg");
    ...
    //     });
    //     // FOR(i,0,T-val[2]) A.v[i]=
    //     polynomial::poly C=A.inv(T+1)*B;
    //     FOR(i,val[2],T) gg[i]=C.v[i];
    // }
    // // get f and get g
    printf("%lld\n",gg[T]);
}
