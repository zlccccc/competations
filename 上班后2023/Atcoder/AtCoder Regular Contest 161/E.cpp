...
void solve() {
    ...
    while (1) {
        FOR_(i,1,n) col[i]=rand()&1;
        T_SAT::init(n+1);
        FOR_(i,1,n) {
            T_SAT::link(edge[i][0],col[i]^1,edge[i][1],col[i]);
            T_SAT::link(edge[i][0],col[i]^1,edge[i][2],col[i]);
            T_SAT::link(edge[i][1],col[i]^1,edge[i][0],col[i]);
            T_SAT::link(edge[i][1],col[i]^1,edge[i][2],col[i]);
            T_SAT::link(edge[i][2],col[i]^1,edge[i][0],col[i]);
            T_SAT::link(edge[i][2],col[i]^1,edge[i][1],col[i]);
        }
        if (!T_SAT::solve()) {
            FOR_(i,1,n) if (col[i]) printf("W"); else printf("B"); puts("");
            break;
        } else {
            // puts("okay");
            // FOR_(i,1,n) if (col[i]) printf("W"); else printf("B"); puts("");
            // FOR_(i,1,n) if (T_SAT::belong[i*2]<T_SAT::belong[i*2+1]) printf("W"); else printf("B"); puts("<- TSAT");
        }
    }
}
int main() {
