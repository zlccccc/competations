...
int main() {
    int n,k;
    fac[0]=1;
    FOR_(i,1,1000000) fac[i]=i*fac[i-1]%M;
    inv[0]=inv[1]=1;
    FOR_(i,2,1000000) inv[i]=(M-M/i)*inv[M%i]%M;
    FOR_(i,1,1000000) inv[i]=inv[i]*inv[i-1]%M;// inv(n!)
    scanf("%d%d",&n,&k);
    FOR_(i,1,n) scanf("%d",&A[i]),Cnt[A[i]]++;
    int ans=0;
    FOR_(i,0,1000000) {
        add_(ans,C(i+k-1,i));
        // printf("add i=%d: C(%d,%d)\n",i,i+k,i);
        if (!Cnt[i]) k--;
        if (k==0) break;
    }
    printf("%d\n",ans);
