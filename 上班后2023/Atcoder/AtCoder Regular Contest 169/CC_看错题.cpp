...
int solve() {
    ...
    // FOR_(i,1,n) printf("%d ",cnt[i]); puts("<- cnt");
    priority_queue<poly> Q;
    FOR_(i,1,n) {
        if (cnt[i]<0) {puts("0"); return 0;}
        if (cnt[i]==0) continue;
        poly cur;
        cur.resize(cnt[i]+1);
        FOR_(x,0,cnt[i]) cur.v[x]=C.g[x].x;
        Q.push(cur);
        for (auto v:cur.v) printf("%lld ",v); puts("<- cur");
    }
    while (Q.size()>1) {
        auto fir=Q.top(); Q.pop();
        auto sec=Q.top(); Q.pop();
        auto fin=fir*sec;
        // fin.resize(min(fin.size(),all+1));
        Q.push(fin);
        for (auto v:fin.v) printf("%lld ",v); puts("<- cur");
    }
    auto top=Q.top();
    printf("%lld\n",(C.f[all]*top.v[all]).x);
    return 0;
