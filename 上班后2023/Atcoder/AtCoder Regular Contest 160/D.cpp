...
#define REP_(I,N) for (ll I=0;I<N;I++)
#define rREP_(I,N) for (ll I=N-1;I>=0;I--)
#define rep_(I,S,N) for (ll I=S;I<N;I++)
#define rrep_(I,S,N) for (ll I=N-1;I>=S;I--)
#define FOR_(I,S,N) for (ll I=S;I<=N;I++)
#define rFOR_(I,S,N) for (ll I=N;I>=S;I--)
...
mint fac[2007],inv[2007];
int cnt=0;
mint multi(mint x,ll y) {
    if (y%mod==0) {cnt++; return x;}
    else return x*y;
}
mint divide(mint x,ll y) {
    if (y%mod==0) {cnt--; return x;}
    else return x/y;
}
int main() {
    ...
    if (m%k!=0) return 0*puts("0");
    poly inc;
    {
        // inc.init(k*(n-k+1),[&](int x){
        //     if (x<k) return 1;
        //     return 0;
        // });
        // // for (int v:inc.v) printf("%d ",v); puts("<- fir");
        // // startTimer();
        // inc=inc.pow(n-k+1);
        inc=poly(1);
        poly base;
        ...
        for (int b=n-k+1;b;b>>=1,base=base*base)
            if (b&1) inc=inc*base;
        //             inline ll powMM(ll a, ll b, ll mod=M) {
        //     ll ret=1;
        //     for (; b; b>>=1ll,a=a*a%mod)
    ...
    // printTimer();
    // for (int v:inc.v) printf("%d ",v);
    mint cur=1,ans;
    m/=k;
    FOR_(i,m+1,m+n-1) cur=multi(cur,i);
    FOR_(i,1,n-1) cur=divide(cur,i);
    REP_(i,min((ll)inc.v.size(),m+1)) {
        // if (i%100000==0) 
            // printf("i=%d: v=%lld ; cur=%lld (%lld)\n",i,inc.v[i],cur.x,m-i);
        // n-i - okay;
        // if (cnt) printf("i=%lld\n",i);
        if (!cnt) ans+=cur*inc.v[i];
        cur=divide(multi(cur,(m-i)),(m-i+n-1));
    }
    printf("%lld\n",ans.x);
}
/*
