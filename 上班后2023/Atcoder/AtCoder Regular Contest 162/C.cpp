...
int main() {
    ...
    FOR(_,1,T){
        ...
        vector<int> A(n+1);
        FOR_(i,2,n) scanf("%d",&f),edge[f].push_back(i);
        FOR_(i,1,n) scanf("%d",&A[i]);
        bool ans=0;
        function<vector<int>(int)> dfs=[&](int x) {
            vector<int> idx; idx.push_back(x);
            for (int v:edge[x]) {
                auto idv=dfs(v);
                for (int id:idv) idx.push_back(id);
            }
            vector<int> cnt(n+1);
            int c=0;
            for (int id:idx) {
            ...
            if (!cnt[k]&&c<=1) {
                int need=0;
                FOR_(i,0,k-1) if (!cnt[i]) need++;
                if (need<=c) ans=1;
                // if (need<=c) puts("ok");
            }
            // printf("solve %d; c=%d\n",x,c);
            // for (int id:idx) printf("%d ",id); puts("<-sub");
            // FOR_(i,0,n) printf("%d ",cnt[i]); puts("<- cnt");
            return idx;
        };
        dfs(1);
        if (ans) puts("Alice");
