...
int main() {
    ...
    ll all=0;
    scanf("%d",&n);
    FOR_(i,1,n) scanf("%lld",&A[i]),all+=A[i];
    ll x=(all-(all%n+n)%n)/n;
    FOR_(i,1,n) A[i]-=x; all-=x*n;
    FOR_(i,1,n) sum[i]=sum[i-1]+A[i];
    // printf("all=%lld\n",all);
    vector<ll> cur(1,0);
    FOR_(i,1,n) {
        vector<ll> nxt(i+1,INFF);
        FOR_(j,0,i-1) FOR_(x,0,1) {
            nxt[j+x]=min(nxt[j+x],cur[j]+abs(sum[i]-j-x));
        }
        cur.swap(nxt);
    }
    printf("%lld\n",cur[all]);
