...
int solve() {
    ...
    cin>>n;
    REP_(i,n) cin>>x,MP[x].first.push_back(i);
    cin>>m;
    REP_(i,m) cin>>x,MP[x].second.push_back(i);
    vector<int> res(n+m+1);
    for (auto &p:MP) {
        auto &cur=p.second;
        if (cur.first.size()*cur.second.size()<=1e6) {
            for (int x:cur.first) for (int y:cur.second) {
                res[y-x+n]--;
            }
        } else {
            poly A,B;
            A.resize(n+1);
            B.resize(m);
            for (int x:cur.first) A.v[n-x]++;
            for (int y:cur.second) B.v[y]++;
            poly C=A*B;
            REP_(i,C.size()) res[i]-=C.v[i];
        }
    }
    rep_(i,1,n+m) {
        int len=min(min(n,m),min(i,n+m-i));
        printf("%d ",len+res[i]);
    }
    return 0;
}
