...
set<string> prepared[6];
void init_all() {
    vector<node> S[11];
    FOR_(i,1,999) FOR_(j,1,999) {
        if (i+j<=999) S[0].push_back({to_string(i)+"+"+to_string(j)+"="+to_string(i+j),0,0});
        if (i-j>0) S[0].push_back({to_string(i)+"-"+to_string(j)+"="+to_string(i-j),0,0});
    }
    int cnt=0,K=5;
    set<string> solved;
    FOR_(i,0,10) {
        for (auto &cur:S[i]) {
            ...
            if (cur.x==cur.y) prepared[cur.x].insert(cur.s);
