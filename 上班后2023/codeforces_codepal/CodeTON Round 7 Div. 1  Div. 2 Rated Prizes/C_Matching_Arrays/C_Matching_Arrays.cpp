...
void solve(int _) {
    ...
    vector<int> res(n+1);
    priority_queue<pair<int,int>> selected;
    vector<int> vec1,vec2;
    rFOR_(i,1,n*2) {
        for (int pos:A[i]) {
            if (vec1.size()<k) vec1.push_back(pos);
            else vec2.push_back(pos);
        }
    }
    reverse(vec1.begin(),vec1.end());
    // for (int v:vec1) printf("%d ",v); puts("<- vec1");
    for (int pos:vec1) {
        auto it=B.begin(); //(V[pos]);
        // printf("find pos1=%d; %d; %d\n",pos,V[pos],*it);
        if ((*it)>=V[pos]) {  // B<A
            puts("NO");
            return;
        }
    ...
    for (int pos:vec2) {
        ...
        res[pos]=*it;
        B.erase(B.lower_bound(*it));
    }
    puts("YES");
    FOR_(i,1,n) printf("%d ",res[i]); puts("");
}
int main() {
    int T = 1;
    scanf("%d",&T);
