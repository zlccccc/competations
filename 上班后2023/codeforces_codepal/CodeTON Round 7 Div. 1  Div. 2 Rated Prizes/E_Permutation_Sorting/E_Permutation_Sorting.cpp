...
inline void update(int x,int val) {
    for (;x<=2000000;x+=lowbit(x)) T[x]+=val;
}
...
void solve(int _) {
    ...
    vector<vector<pii>> Ask(n*2+1);
    FOR_(i,1,n) {
        int l=i,r=A[i];
        if (r<l) r+=n;
        // printf("%d->%d\n",l,r);
        ans[A[i]]=r-l;
        Ask[r].push_back({l,A[i]});
        Q[r].push_back(l);
        if (r<=n) Q[r+n].push_back(l+n);
    }
    FOR_(i,1,n+n) T[i]=0;
    FOR_(i,1,n+n) {
        for (auto &now:Ask[i]) {
            ans[now.second]+=query(now.first,i);
            // printf("ask %d -> %d: query=%d\n",now.first,i,query(now.first,i));
        }
        for (int v:Q[i]) {
            update(v,-1);
            // printf("update %d -> %d\n",v,i);
