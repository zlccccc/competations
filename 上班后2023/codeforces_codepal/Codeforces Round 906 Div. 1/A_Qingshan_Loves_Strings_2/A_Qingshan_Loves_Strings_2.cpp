...
void solve(int _) {
    ...
    cin>>n>>s;
    int k=0;
    for (char c:s) if (c=='0') k++; else k--;
    if (k) {
        puts("-1");
        return;
    }
    vector<int> ins;
    function<void(string s,int l,int r)> solve=[&](string s,int l,int r) {
        // cout<<s<<" "<<l << " "<<r << endl;
        if (l>r) return;
        if (s[l]!=s[r]) solve(s,l+1,r-1);
        else if (s[l]=='0'){
            string nxt=s.substr(0,r+1)+"01"+s.substr(r,s.size()-r-1);
            ins.push_back(r+1);
            solve(nxt,l,r+2);
        } else {
            string nxt=s.substr(0,l)+"01"+s.substr(l,s.size()-l);
            ins.push_back(l);
            solve(nxt,l,r+2);
        }
    };
