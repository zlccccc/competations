...
vector<int> solve(int n) {
    ...
    function<void(int, int, int)> work = [&](int p,int l,int r) {
        ...
        if (l+1==r) {
            if (l==1||l==2) ans[l]=1;
            else add_(ans[l],M-(ll)(l-1)*ans[l-1]%M);
        ...
        // if (r-l<2) {
            rep(i,l,r) {
                ...
                rep(j,l,i) { // l<=j<i; no i-j limit
                    ...
                    add_(ans[i],(ll)ans[j]*ans[i-j]%M*C(i-1,j-1)%M*2%M);
                }
                rep(j,1,min(l,i-l+1)) { // j<l and l<=i-j<i;
                    // printf("l,r=%d %d; i=%d j=%d i-j=%d\n",l,r,i,j,i-j);
                    add_(ans[i],(ll)ans[j]*ans[i-j]%M*C(i-1,j-1)%M*2%M);
                }
                add_(ans[i],M-(ll)(i-1)*ans[i-1]%M);
            }
            return;
        }
        int m=(l+r)>>1;
        ...
        // printf("work p=%d: sol 0 ans=%d l,r=%d->%d\n",p,ans[l],l,r);
        {
            ...
            rep(i,l,m) xv.push_back((ll)ans[i]*inv[i]%M*2%M); // x=l->m-1
