...
namespace LB {
    ...
    L_B merge(const L_B &A,const L_B &B) {
        L_B ret=A;
        REP_(i,MaxBit) if (B.b[i]) ret.insert(B.b[i]);
        return ret;
    }
}
using namespace LB;
...
void dfs(int x,int f,int depth) {
    ...
    rep_(i,1,18) {
        ...
        T[x][i]=merge(T[x][i-1],T[fa[x][i-1]][i-1]);
    }
...
int main() {
    ...
    FOR_(i,1,q) {
        ...
        // printf("f=%d; d=%d; %d\n",f,dep[f],dep[x]);
        L_B cur;
        rREP_(i,20) if (dep[x]-(1<<i)>=dep[f]) cur=merge(cur,T[x][i]),x=fa[x][i];
        rREP_(i,20) if (dep[y]-(1<<i)>=dep[f]) cur=merge(cur,T[y][i]),y=fa[y][i];
        // printf("x=%d; y=%d; f=%d\n",x,y,f);
        cur.insert(A[f]);
