...
int main() {
    ...
    FOR(_,1,T){
        ...
        FOR_(x,0,10) {
            if (!k) break;
            // printf("solve x=%d; k=%lld; s=%lld\n",x,k,s);
            ans=max(ans,s*k);
            if (MP.count(s%10)) {
                pii cur=MP[s%10];
                ll c=cur.first,v=cur.second;
                if (v) {
                    // cur.first = count; cur.second = size
                    ...
                    // printf("%d -> %d\n",l,r);
                    for (ll x=l;x<=r;x++) {
                        x=max(x,0ll); x=min(x,k);
                        ans=max(ans,(s+v*x)*(k-c*x));
                    }
                }
            }
            MP[s%10]={0,0};
