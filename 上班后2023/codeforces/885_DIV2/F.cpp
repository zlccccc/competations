...
vector<int> solve(vector<int> x) {
    ...
    REP_(i,(int)x.size()) ret[i]=x[i]^x[(i+1)%x.size()];
    return ret;
}
int main() {
    int n;
    scanf("%d",&n);
    vector<int> A(n);
    int allor=0,allxor=0;
    REP_(i,n) scanf("%d",&A[i]),allxor^=A[i],allor|=A[i];
    if (!allor) return 0*puts("0"); // zero
    // if (allxor) return 0*printf("%d\n",n); // not okay
    int ans=0;
    while (n>1) {
        int notsame=0;
        REP_(i,n/2) if (A[i]!=A[i+n/2]) notsame=1;
        if (notsame) {
            ans+=n/2;
            REP_(i,n/2) A[i]^=A[i+n/2];
        }
        n/=2;
    }
    if (A[0]) ans++;
