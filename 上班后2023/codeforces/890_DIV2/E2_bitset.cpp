...
int f[maxn];
template<int S=1>
ll solve(int sum,map<int,int> MP) {
    if (S<sum-S) return solve<min(S*2,1000000)>(sum,MP);
    bitset<S+1> bit;
    bit.set(0);
    for (auto &p:MP) {
    ...
    ll cur=0;
    // printf("S=%d\n",S);
    // FOR_(i,0,S) if (bit[i]) printf("%d ",i); puts("<-bit");
    FOR_(i,0,S) if (bit[i]) cur=max(cur,(ll)i*(sum-i));
    return cur;
}
int sz[maxn];
ll dfs(int x) {
