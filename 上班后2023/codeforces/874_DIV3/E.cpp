...
int main() {
    ...
    FOR(_,1,T){
        ...
        FOR_(i,1,n) fa[i]=i,cnt[i]=1,S[i].clear();
        FOR_(i,1,n) {
            ...
            if (x!=y) {
                ...
                for (int v:S[x]) S[y].insert(v);
                S[x].clear();
            }
        }
        int maybe=0,must=0;
        FOR_(i,1,n) if (getfa(i)==i) {
            if (cnt[i]!=S[i].size()||cnt[i]<=2) maybe++;
            else must++;
        }
        // printf("maybe=%d must=%d\n",maybe,must);
        printf("%d %d\n",(maybe!=0)+must,maybe+must);
    }
}
/*
5
