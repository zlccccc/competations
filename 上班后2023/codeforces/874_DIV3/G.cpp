...
int D[maxn],cnt[maxn]; // D:du
vector<pii> edge[maxn];
int rem[maxn],mark[maxn];
int solve() {
    int n;
    scanf("%d",&n);
    FOR_(i,1,n) edge[i].clear(),cnt[i]=D[i]=mark[i]=0;
    FOR_(i,1,n-1) {
        rem[i]=0;
        int x,y;
        scanf("%d%d",&x,&y);
    ...
    queue<int> Q;
    FOR_(i,1,n) if (D[i]==1) Q.push(i);
    while (Q.size()) {
        int x=Q.front(); Q.pop();
        mark[x]=1;
        // printf("bfs %d: %d\n",x,cnt[x]);
        if (cnt[x]>2) return 0*puts("-1");
        for (auto &p:edge[x]) {
    ...
    printf("%d\n",n/3-1);
    FOR_(i,1,n-1) if (rem[i]) printf("%d ",i); puts("");
    return 1;
}
int main() {
    int T,_; T=1;
