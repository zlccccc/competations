...
int solve() {
    ...
    vector<int> A(n+1);
    vector<ll> sum(n+1);
    vector<int> prevpos(n+2);
    FOR_(i,1,n) scanf("%d",&A[i]),sum[i]=A[i]+sum[i-1];
    int l=1;
    ll res=0;
    FOR_(i,1,n) {
        while (sum[i]-sum[l-1]>x) l++;
        int curl=i;
        prevpos[min(A[i],n+1)]=i;
        FOR_(j,0,n) {
            if (curl<l) break;
            // l<=x的时候; ans才会>=j
            int nxtl=max(l-1,min(curl,prevpos[j]));
            // printf("solve 1 %d,%d -> %d: %d\n",nxtl,curl,i,j);
            res+=(ll)(curl-nxtl)*j;
            curl=nxtl;
        }
    }
