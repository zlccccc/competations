...
int solve() {
    ...
    vector<int> vec;
    FOR_(i,1,m) Id[Cnt[i]].push_back(i);
    rFOR_(k,1,n) {
        auto update=[&](vector<int> &fir,vector<int> &sec,int w) {
            const int limit=3000;
            if (fir.size()>=limit&&sec.size()>=limit) {
                // poly-solve
                poly A,B;
                A.resize(m+1);
                B.resize(m+1);
                for (int x:fir) A.v[x]++;
                for (int x:sec) B.v[x]++;
                // REP_(i,A.size()) printf("%d ",A[i]); puts("<- A");
                // REP_(i,B.size()) printf("%d ",B[i]); puts("<- B");
                poly C=A*B;
                // REP_(i,C.size()) printf("%d ",C[i]); puts("<- C");
                REP_(i,C.size()) res[i]+=C[i]*k*w;
            } else {
                for (int x:fir) for (int y:sec) {
                    res[x+y]+=k*w;
                }
            }
        };
        update(Id[k],Id[k],1);
        for (int v:Id[k]) res[v+v]-=k;
        update(Id[k],vec,2);
        for (int v:Id[k]) vec.push_back(v);
    }
    // FOR_(i,1,m*2) printf("%d ",res[i]); puts("<- res");
    FOR_(i,1,q) {
        int x;
        scanf("%d",&x);
        int cur=res[x]/2;
        if (x%2==0) cur+=Cnt[x/2]/2;
        printf("%d\n",cur);
    }
    return 0;
}
