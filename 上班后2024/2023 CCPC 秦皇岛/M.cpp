...
mint f[maxn],g[maxn];  // f:不连通方案数; g:连通
bool vis[maxn];
bool red[maxn];
void dfs(int x,int fa) {
    ...
    for (int v:edge[x]) {
        ...
        if (red[v]) {
            dfs(v,x);
            mint nf=f[x]*f[v]+f[x]*g[v]*2;
            mint ng=f[x]*g[v]+g[x]*g[v]*2+g[x]*f[v];
            g[x]=ng; f[x]=nf;
        } else {
            g[x]=g[x]*2+f[x]; // g:已经连通; f:还没连通
            f[x]=f[x]*2; // 选一条
        }
    }
}
int solve() {
    ...
    FOR_(_,1,n-1) {
        ...
        red[v]=1;
        mint res=1;
        FOR_(i,1,n) f[i]=g[i]=0,vis[i]=0;
        FOR_(i,1,n) if (red[i]&&!vis[i]) {
            dfs(i,0);
            res*=g[i];
            // printf("solve v=%d; g=%lld\n",i,g[i].x);
        }
        printf("%lld\n",res.x);
        // puts("");
    }
