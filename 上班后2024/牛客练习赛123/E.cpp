...
int solve() {
    ...
    mint inv2=mint(1)/mint(2),inv4=mint(1)/mint(4);
    mint res=0;
    // FOR_(i,1,min({n,m,p})) {
    //     mint cur=mint(i)*i*i;
    //     cur*=inv2*(n/i)*((n/i)+1);
    ...
    // }
    auto C3=[&](ll x) {
        return mint(x)*mint(x+1)*mint(x)*mint(x+1)*inv4;
    };
    for (ll i=1;i<=min({n,m,p});i++) {
        ll r=min({n/(n/i),m/(m/i),p/(p/i)});
        // printf("r=%lld\n",r);
        // mint cur=mint(i)*i*i;
        // printf("%lld %lld\n",C3(r).x,C3(i-1).x);
        mint cur=C3(r)-C3(i-1);
        cur*=inv2*(n/i)*((n/i)+1);
        cur*=inv2*(m/i)*((m/i)+1);
        cur*=inv2*(p/i)*((p/i)+1);
        // printf("cur=%lld\n",cur.x);
