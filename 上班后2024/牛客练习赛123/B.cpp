...
int solve() {
    ...
    FOR_(i,1,n) Pre[i]=max(Pre[i-1],A[i]);
    rFOR_(i,1,n) Suf[i]=max(Suf[i+1],A[i]);
    // 除了这种颜色之外都可以丢完;
    FOR_(i,1,n) {
        ll MAX=max(Pre[i-1],Suf[i+1]),ALL=all-A[i];
        ll remain=max(ALL&1,MAX*2-ALL);
        if (A[i]>remain) printf("1 ");
        else printf("0 ");
    } puts("");
    return 0;
}
