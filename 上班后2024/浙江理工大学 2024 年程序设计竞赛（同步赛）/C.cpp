...
void init() {
    ...
    FOR_(i,1,1000) {
        mint cur=0;
        // FOR_(l,0,i-1) printf("%d %d\n",l,i-l-1);
        FOR_(l,0,i-1) if (l<=i-l-1) cur+=dp[l]*dp[i-l-1];
        if (i%2==1) {
            // left=right
            cur-=(dp[(i-1)/2]*dp[(i-1)/2]-dp[(i-1)/2])/2;
        }
        dp.push_back(cur);
    }
    // FOR_(i,0,10) printf("%lld ",dp[i].x); puts("<- dp");
    // for (auto x:dp) printf("%lld ",x.x);
}
int solve() {
    int x;
    scanf("%d",&x);
