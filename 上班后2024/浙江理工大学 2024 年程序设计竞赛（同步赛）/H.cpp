...
int solve() {
    int n;
    scanf("%d",&n);
    vector<ll> t1(n),t2(n),res(n);
    REP_(i,n) scanf("%lld%lld",&t1[i],&t2[i]);
    ll t=0;
    REP_(x,n) {
        int cur=-1;
        REP_(i,n) if (!res[i]) {
            auto cmp=[&](int x,int y) {
                if (x==-1) return y;
                if (y==-1) return x;
                __int128 waitx=max(0ll,t-t1[x]),waity=max(0ll,t-t1[y]);
                if (waitx*t2[y]!=waity*t2[x]) return waitx*t2[y]>waity*t2[x]?x:y;
                if (t1[x]!=t1[y]) return t1[x]<t1[y]?x:y;
                return x<y?x:y;
            };
            cur=cmp(cur,i);
        }
