...
int solve() {
    ...
    FOR_(i,1,n) {
        int x=i,y;
        if (i==n) y=n-1; else y=i+1,i++;
        int cur=ask(x,y);
        if (!cur) continue;
        int rx=1,ry=1,askd=0;
        FOR_(i,1,n) if (i!=x&&i!=y) {
            if (rx&&ry) {
                int linkx=ask(i,x);
                int linky=ask(i,y);
                if (linkx==linky) return 0*puts("! 1");
                rx&=linkx; ry&=linky;
            } else {
                int link;
                if (rx) link=ask(i,x); else link=ask(i,y);
                if (link) return 0*puts("! 2");
                else return puts("! 1");
            }
        }
    }
