...
mint prob[maxn];
int vis[maxn];
void dfs(int x,int dep,int fa) { // must dfs!
    vis[x]=1;
    // printf("dfs %d -> %d\n",fa,x);
    prob[x]=prob[fa]*inv[dep];
    for (int v:edge[x]) {
        dfs(v,dep+1,x);
    }
}
...
void solve(int x) {
    vector<int> cir;
    cir.clear();
    while (E[x]&&!vis[E[x]]) x=E[x],vis[x]=1;
    while (E[x]&&vis[E[x]]==1) {
        vis[E[x]]=2; cir.push_back(x); x=E[x];
    }
    // rREP_(i,cir.size()) printf("%d ",cir[i]); puts("<- cir");
    if (cir.size()!=1) {
        rREP_(i,cir.size()) prob[cir[i]]=0;
    }
    for (int v:cir) for (int y:edge[v]) if (vis[y]!=2) dfs(y,2,v);
}
int A[maxn];
int B[maxn],W[maxn];
int solve() {
    ...
    // FOR_(i,1,n) scanf("%d",&W[i]);
    read(n);
    FOR_(i,1,n) read(A[i]);
    FOR_(i,1,n) read(B[i]);
    FOR_(i,1,n) read(W[i]);
    FOR_(i,1,n) edge[i].clear();
    FOR_(i,1,n) vis[i]=0,prob[i]=0,E[i]=0;
    FOR_(i,1,n) {
        if (B[i]==i||A[i]>=A[B[i]]+W[B[i]]) {prob[i]=0; E[i]=i; continue;}  // 一定不会upd
        if (A[i]<A[B[i]]) {prob[i]=1; E[i]=i; continue;}  // 一定upd
        // printf("add edge %d <- %d\n",i,B[i]);
        edge[B[i]].push_back(i); E[i]=B[i];
    }
    FOR_(i,1,n) if (!vis[i]) solve(i);
    // FOR_(i,1,n) printf("%lld ",prob[i]); puts("<- prob");
    FOR_(i,1,n) print((prob[i]*W[i]+A[i]).x),print(" "); println("");
    return 0;
}
int main() {
    inv[0]=inv[1]=1;
    FOR_(i,2,1000000) inv[i]=inv[mod%i]*(mod-mod/i);
    int T;
    T = 1;
    // cin>>T;
    // scanf("%d", &T);
