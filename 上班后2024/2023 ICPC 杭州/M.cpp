...
int solve() {
    ...
    L[1]=0;
    FOR_(i,2,n) if (A[i]<A[i-1]) L[i]=L[i-1]+1; else L[i]=0;
    R[n]=0;
    rFOR_(i,1,n-1) if (A[i]<A[i+1]) R[i]=R[i+1]+1; else R[i]=0;
    // FOR_(i,1,n) printf("%d ",L[i]); puts("<-L");
    // FOR_(i,1,n) printf("%d ",R[i]); puts("<-R");
    double res=0;
    FOR_(i,2,n-1) if (L[i]&&R[i]) {
        int l=i-L[i],r=i+R[i];
        // printf("%d->%d->%d\n",l,i,r);
        FOR_(x,l,i-1) max_(res,(double)(sum[r]-sum[x-1])/(r-x+1));
        FOR_(x,i+1,r) max_(res,(double)(sum[x]-sum[l-1])/(x-l+1));
    }
    printf("%.15f\n",res);
    return 0;
}
