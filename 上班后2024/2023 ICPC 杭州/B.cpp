...
typedef bitset<250007> Bit;
const int Large=1000;
vector<int> items[250007];  // color
map<int,Bit> largeblock;
int result[250007];
int solve() {
    int n,q;
    // scanf("%d%d", &n,&q);
    ...
    Bit all,not_selected;
    not_selected.flip();
    FOR_(i,1,n) {
        // scanf("%d%d",&x[i],&c[i]);
        read(x[i]); read(c[i]);
        items[c[i]].push_back(x[i]);
        all.set(x[i]);
    }
    FOR_(i,1,250000) {
        if (items[i].size()>Large) {
            Bit cur; cur.reset();
            for (int v:items[i]) cur.set(v);
            largeblock[i]=move(cur);
        }
    }
    FOR_(i,1,n) {
        if (largeblock.count(c[i])) all^=largeblock[c[i]];
        else for (int v:items[c[i]]) all.flip(v);
        Bit cur=(all>>x[i])&not_selected;
        for (int x=cur._Find_first();x<250007;x=cur._Find_next(x)) {
            // printf("i=%d; x=%d\n",i,x);
            result[x]=i;
            not_selected.flip(x);
        }
        // printf("i=%d\n",i);
        // for (int x=all._Find_first();x<250007;x=all._Find_next(x)) printf("%d ",x); puts("<- all useful");
