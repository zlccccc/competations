...
typedef pair<int,array<int,26>> Limits;  // Limits[26]
vector<int> limit[maxn];
int solve() {
    int n;
    scanf("%d",&n);
    vector<int> len(n);
    vector<Limits> items(n);
    REP_(i,n) {
        string str;
        cin>>str;
        items[i].first=str.size();
        for (char c:str) items[i].second[c-'a']++;
    }
    priority_queue<Limits> limits; // length, limit
    rrep_(k,1,n) {
        if (k) {
            // merge Node x, Node x-1
            limits.push(items[k]);
            while (limits.size()&&limits.top().first>=items[k-1].first) {  // 变短
                auto cur=limits.top(); limits.pop();
                // printf("%d | ",cur.first);
                // REP_(i,26) printf("%d ",cur.second[i]); puts(" < merge current limits");
                Limits newlimit;
                int repeat=cur.first/items[k-1].first;
                newlimit.first=cur.first%items[k-1].first;
                REP_(i,26) newlimit.second[i]+=cur.second[i]-items[k-1].second[i]*repeat;
                REP_(i,26) if (newlimit.second[i]<0) return 0*puts("NO");
    ...
    while (limits.size()) {
        ...
        // printf("%d | ",cur.first);
        // REP_(i,26) printf("%d ",cur.second[i]); puts(" < limits");
        auto top=limits.top();
        REP_(i,26) {
            if (top.second[i]>cur.second[i]) return 0*puts("NO");
            REP_(t,cur.second[i]-top.second[i]) str+='a'+i;
        }
    }
    ...
    cout<<str<<"\n";
    rep_(i,1,n) {
        while (str.size()>items[i].first) str.pop_back();
        while (str.size()<items[i].first) str.push_back(str[str.size()%items[i-1].first]);
        cout<<str<<"\n";
    }
    return 0;
}
