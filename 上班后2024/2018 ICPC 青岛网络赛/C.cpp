...
int solve() {
    int n;
    cin>>n;
    vector<bitset<256>> mark(n+1);
    vector<tuple<string,int,int>> vec(n+1);
    FOR_(i,1,n) {
        cin>>std::get<0>(vec[i]);
        if (get<0>(vec[i])=="add") cin>>get<1>(vec[i]);
        else cin>>get<1>(vec[i])>>get<2>(vec[i]);
    }
    int cur=1;
    int r=0;
    while (cur<=n) {
        // cout<<"operation "<<cur<<" "<<r<<endl;
        if (mark[cur][r]) {
            cout<<"No"<<endl;
            return 0;
        }
        mark[cur].set(r);
        if (get<0>(vec[cur])=="add") {
            r+=get<1>(vec[cur]);
            r%=256;
            cur++;
        } else if (get<0>(vec[cur])=="beq") {
            int v=get<1>(vec[cur]),k=get<2>(vec[cur]);
            if (r==v) cur=k;
            else cur++;
        } else if (get<0>(vec[cur])=="bne") {
            int v=get<1>(vec[cur]),k=get<2>(vec[cur]);
            if (r!=v) cur=k;
            else cur++;
        } else if (get<0>(vec[cur])=="blt") {
            int v=get<1>(vec[cur]),k=get<2>(vec[cur]);
            if (r<v) cur=k;
            else cur++;
        } else if (get<0>(vec[cur])=="bgt") {
            int v=get<1>(vec[cur]),k=get<2>(vec[cur]);
            if (r>v) cur=k;
            else cur++;
        } else {
