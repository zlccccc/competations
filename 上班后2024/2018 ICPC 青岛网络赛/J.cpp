...
int solve() {
    ...
    auto solve=[&](ll time) {
        ...
        ll x=0,y=0;
        while (min(x,y)<=time) {
            // cout<<"solve "<<x<<" "<<y<<": "<<cnt<<" last="<<last<<endl;
            if (x<=y) {  // x press
                cnt+=b-(x>last);
                last=x+v;
                x+=a;
        ...
        // cout<<time<<": "<<cnt<<" "<<(last>t)<<endl;
        return pll{cnt,(last>time)};
    };
    ll multi=a*c;
    if (multi>t) cout<<solve(t).first<<endl;
    else {
        auto cir=solve(multi-1),oth=solve(t%multi);
        cout<<(cir.first+cir.second)*(t/multi)+oth.first<<endl;
    }
    return 0;
}
int main() {
