...
int solve() {
    ...
    rFOR_(i,1,n) {
        ...
        if (edge[x].size()>=i-1) {
            MAX=i;  // x nodes
            FOR_(_,1,i) p1.push_back(id[_]);
            FOR_(_,i+1,n) p2.push_back(id[_]);
            break;
        }
    }
    // for (int v:p1) printf("%d ",v); puts("<- part1");
    // for (int v:p2) printf("%d ",v); puts("<- part2");

    {
        int res=1;
        for (int x:p2) if (edge[x].size()==MAX-1) res++;
        printf("%d ",res);
    }
    {
        int MIND=INF;
        for (int x:p1) MIND=min(MIND,(int)edge[x].size());
        // printf("MIND=%d MAX=%d\n",MIND,MAX);
        if (MIND==MAX-1) {
            int res=0;
            for (int x:p1) if (edge[x].size()==MAX-1) res++;
            printf("%d\n",res);
        } else if (MIND==MAX) {
            int res=1;
            for (int x:p1) if (edge[x].size()==MAX) res++;
            printf("%d\n",res);
        } else {
            printf("%d\n",1);
        }
