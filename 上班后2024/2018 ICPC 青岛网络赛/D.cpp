...
int solve() {
    ...
    int black=0,current_black=0,component=0;
    ...
    vector<vector<int>> ins(n+2),del(n+2);
    vector<pii> range(k+1);
    FOR_(i,1,k) {
        int r1,c1,r2,c2;
    ...
    FOR_(dep,1,n) {
        ...
        for (int id:ins[dep]) {
            auto [l,r]=range[id];
            // printf("check upperbound %d %d %d\n",l,r,id);
            auto it=S.upper_bound({l,INF,INF});
            if (it!=S.begin()) it--;
            while (it!=S.end()&&(*it)[0]<=r) {
                if (l<=(*it)[1]) merge((*it)[2],id);
                it++;
            }
        }
        ...
        for (int id:ins[dep]) {
            auto [l,r]=range[id];
            // printf("insert and check l/r %d %d %d\n",l,r,id);
            current_black+=r-l+1; component++;
            S.insert({l,r,id});
            auto it=S.find({l,r,id});
            if (it!=S.begin()&&(*prev(it))[1]==l-1) merge((*prev(it))[2],id);
            if (next(it)!=S.end()&&(*next(it))[0]==r+1) merge((*next(it))[2],id);
        }
        black+=current_black;
        printf("%d %d\n",black,component);
    }
