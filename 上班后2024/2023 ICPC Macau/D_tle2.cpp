...
vector<int> red[maxn];
vector<int> blue[maxn];
vector<int> nodes;
int id[maxn],tot;
set<vector<int>> items[maxn];
vector<vector<int>> itemRed,itemBlue;
int res=0;
void dfs(int x,int fa) {
    ...
    for (auto it=items[x].begin();it!=items[x].end();) {
        itemRed.push_back(*it);
        // for (int v:*it) printf("%d ",v); puts("  <- cal");
        if ((*it).size()==4) it=items[x].erase(it);
        else it++;
    }
}
int solve() {
    ...
    FOR_(i,1,m) {
        ...
        else blue[x].push_back(y),blue[y].push_back(x);
    }
    swap(red,blue);
    FOR_(i,1,n) if (!id[i]) dfs(i,0);
    itemRed.swap(itemBlue);
    swap(red,blue);
    // puts("swap");
    FOR_(i,1,n) id[i]=0;
    FOR_(i,1,n) if (!id[i]) dfs(i,0);
    sort(itemBlue.begin(),itemBlue.end());
    sort(itemRed.begin(),itemRed.end());
    auto it=itemBlue.begin();
    for (auto &item:itemRed) {
        while (it!=itemBlue.end()&&item>*it) it++;
        if (item==*it) res++;
    }
    cout<<res;
    return 0;
