...
int solve() {
    ...
    if (m>3e5) return 0*puts("NO");
    FOR_(i,1,k) {
        int l,r,k;
        scanf("%d%d%d",&l,&r,&k);
        seg[k].push_back({l,r});
    }
    deque<pii> arr;  // {l,r,idx}
    int edg=0,nod=0;
    FOR_(i,1,m) {
        ...
        deque<pii> cur;
        {
            int prev=1;
            for (auto [l,r]:seg[i]) {
                if (l-1>=prev) cur.push_back({prev,l-1});
                prev=r+1;
            }
            if (prev<=n) cur.push_back({prev,n});
        }
        {
            for (auto [l,r]:cur) {
                edg+=r-l;
                nod+=r-l+1;
                // calculate overlap
                while (arr.size()&&arr[0].first<=r) {
                    int x=arr[0].first,y=arr[0].second;
                    edg+=max(0,min(y,r)-max(x,l)+1);
                    // printf("link: %d -> %d\n",max(x,l),min(y,r));
                    if (y>r) break;
                    arr.pop_front();
                }
            ...
            // printf("i=%d\n",i);
            // for (auto [l,r]:cur) printf("%d->%d ",l,r); puts(" <- nodes");
        }
        arr.swap(cur);
    }
    // printf("nod=%d; edg=%d\n",nod,edg);
    if (nod==edg+1) puts("YES");
    else puts("NO");
    return 0;
}
int main() {
