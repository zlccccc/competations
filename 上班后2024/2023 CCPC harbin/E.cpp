...
int solve() {
    ...
    vector<ll> W(n+1),lim(n+1);
    vector<int> id(n+1);
    FOR_(i,1,n) scanf("%lld%lld%lld",&A[i],&B[i],&W[i]);
    ll l=0,r=INFF;
    ll allB=0;
    FOR_(i,1,n) allB+=B[i];
    while (l+1<r) {
        auto check=[&](ll mid) {
            FOR_(i,1,n) id[i]=i;
            FOR_(i,1,n) lim[i]=mid/W[i]-allB-B[i]-(A[i]-B[i]);
            sort(id.begin()+1,id.begin()+1+n,[&](int x,int y) {
                bool negx=(A[x]-B[x]<0),negy=(A[y]-B[y]<0);
                if (negx!=negy) return negx;
                if (negx&&negy) return lim[x]>lim[y];  // judge last
                return lim[x]+(A[x]-B[x])<lim[y]+(A[y]-B[y]);
            });
            ll now=0;
            FOR_(i,1,n) {
                int x=id[i];
                if (now>lim[x]) return false;
                now+=A[x]-B[x];
            }
            return true;
        };
