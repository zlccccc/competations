...
int solve() {
    ...
    FOR_(i,1,n) if (!sz[i]) {
        function<void(int,int)> dfs2=[&](int x,int fa) {
            ...
            {
                int cursg=0;
                if (fa) cursg^=getsg(sz[i]-sz[x]);
                for (int v:edge[x]) if (v!=fa) cursg^=getsg(sz[v]);
                // printf("remove %d: %d ^ %d\n",x,cursg,getsg(sz[i]));
                res[cursg^getsg(sz[i])]++;
            }
            {
                for (int v:edge[x]) if (v!=fa) {
                    // printf("remove %d - %d: %d ^ %d\n",x,v,getsg(sz[v])^getsg(sz[i]-sz[v]),getsg(sz[i]));
                    res[getsg(sz[v])^getsg(sz[i]-sz[v])^getsg(sz[i])]++;  // remove [x,v]
                }
            }
        };
        dfs(i,0);
