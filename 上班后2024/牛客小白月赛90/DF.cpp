...
int solve() {
    ...
    vector<map<int,mint>> MP(n+1);
    MP[0][0]+=1;
    FOR_(l,1,n) {
        for (int r:R[l]) {
            vector<tuple<int,int,mint>> upd;
            // FOR_(i,l-1,n) {
            for (int i:useful) if (i>=l-1) {
                for (auto [cur,cnt]:MP[i]) {  // m个有值的位置
                    int Min=max(min(cur,r),i),Max=max(cur,r);
                    upd.push_back({Min,Max,cnt});  // choose
                    // upd.push_back({l,cur,cnt});  // not choose
                }
            }
            for (auto [Min,Max,cnt]:upd)
                MP[Min][Max]+=cnt;
        }
    }
    // FOR_(l,0,n) {
    //     for (auto [cur,cnt]:MP[l]) printf("[%d:%lld] ",cur,cnt.x); puts("<- dp");
    // }
    auto res=MP[n][n];
    printf("%lld\n",res.x);
    return 0;
