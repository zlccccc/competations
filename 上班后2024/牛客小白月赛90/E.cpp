...
int solve() {
    ...
    vector<ll> A(n+1);
    vector<ll> B(n+1);
    FOR_(i,1,n) cin>>A[i],A[i]+=A[i-1];
    FOR_(i,1,n) cin>>B[i];  // last>=A[i]+B[i]: 可以使用B[i]
    FOR_(_,1,q) {
        int x;
        cin>>x;
        ...
        ll all=0,res=INFF;
        FOR_(i,1,n) {
            if (PQ.size()==x&&PQ.top()>=B[i])
                all-=PQ.top(),PQ.pop();
            if (PQ.size()<x) PQ.push(B[i]),all+=B[i];
            // printf("i=%d: %lld\n",i,all+A[i]);
            if (PQ.size()==x) res=min(res,all+A[i]);
        }
        cout<<res<<"\n";
    }
    return 0;
