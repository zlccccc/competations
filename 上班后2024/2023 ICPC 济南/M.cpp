...
int solve() {
    ...
    REP_(i,n) used[i]=0;
    REP_(i,top) used[id[i]]=1;

    // REP_(i,top) printf(" > %f %f\n",s[i].x,s[i].y);
    // REP_(i,top) printf("%d ",id[i]); puts("<- id");
    int res=1;
    REP_(i,n) {
        if (used[i]) continue;
        vector<int> pts;
        REP_(j,n) if (j!=i) pts.push_back(j),angle[j]=(p[j]-p[i]).angle();
        sort(pts.begin(), pts.end(),[&](int x,int y) {
            return angle[y]>angle[x];
        });
        if (pts.size()) pts.push_back(pts[0]);
        // printf("i=%d  (%f %f)\n",i,p[i].x,p[i].y);
        // REP_(i,pts.size()-1) printf("%d(%f %f;%d) ",pts[i],p[pts[i]].x,p[pts[i]].y,(used[pts[i]&&used[pts[i+1]]])); puts("<- ids");
        REP_(i,pts.size()-1) if (used[pts[i]]&&used[pts[i+1]]) res++;
    }
    printf("%d\n",res);
    return 0;
}
