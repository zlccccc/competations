...
int solve() {
    ...
    scanf("%d",&n);
    vector<int> A(n+2);
    FOR_(i,1,n) scanf("%d",&A[i]),m=max(m,A[i]);
    vector<vector<int>> ind(m+1);
    FOR_(i,1,n) ind[A[i]].push_back(i);
    vector<int> fa(n+2);
    FOR_(i,1,n) fa[i]=i;
    ...
    // vector<int> cur(m+2);  // suffix sum
    vector<ll> res(m+2);  // suffix sum
    auto upd=[&](int x,int R,int op) {
        // printf("update %d %d %d\n",x,R,op);
        x--; R--;
        for (int l=1;l<=R;l++) {
            int val=x/l;
    ...
    rFOR_(i,1,m) {
        ...
        for (int id:ind[i]) {
            upd(A[id],A[id],1);
            if (A[id-1]>=A[id]) merge(id,id-1);
            if (A[id+1]>=A[id]) merge(id,id+1);
        }
        // FOR_(j,1,m) printf("%d ",cur[j]); puts("<- cur");
    }
    FOR_(i,1,m) printf("%lld ",res[i]+1); puts("");
    return 0;
}
int main() {
