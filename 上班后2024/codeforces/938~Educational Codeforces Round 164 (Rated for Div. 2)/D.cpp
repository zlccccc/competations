...
int solve() {
    ...
    sort(A.begin(),A.end());
    vector<array<mint,3>> dp(5002,{0,0,0});  // 左边set个数; 右侧都不可选; {sum,cnt0,cnt1}
    dp[0]={0,1,0};
    mint res=0;
    for (int sz:A) {
        rFOR_(i,0,5001) {
            int nxt=min(5001,i+sz);
            auto &[sum,cnt0,cnt1]=dp[i];
            // choose sz
            mint newsum;
            if (sz&1) {
                newsum=sum+cnt1*(sz/2)+cnt0*((sz+1)/2);
                dp[nxt][0]+=newsum;
                dp[nxt][1]+=cnt1;
                dp[nxt][2]+=cnt0;
            } else {
                newsum=sum+cnt1*(sz/2)+cnt0*(sz/2);
                dp[nxt][0]+=newsum;
                dp[nxt][1]+=cnt0;
                dp[nxt][2]+=cnt1;
            }
            if (i<sz) res+=(cnt0+cnt1)*sz;
            else res+=newsum;
        }
        // int i=5000;
        // while (dp[i]==array<mint,3>{0,0,0}) i--;
        // printf("res=%lld\n",res);
        // FOR_(j,0,i) printf("[%lld %lld %lld] ",dp[j][0],dp[j][1],dp[j][2]); puts("");
    }
    printf("%lld\n",res.x);
    return 0;
}
