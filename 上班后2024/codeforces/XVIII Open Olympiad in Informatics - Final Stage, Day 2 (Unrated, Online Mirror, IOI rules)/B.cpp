...
int solve() {
    int n;
    scanf("%d", &n);
    FOR_(i,1,n) scanf("%lld",&D[i]);
    FOR_(i,1,n) scanf("%lld",&L[i]);
    FOR_(i,1,n) scanf("%lld",&R[i]);
    scanf("%lld%lld",&L[0],&R[0]);
    rFOR_(i,0,n) D[i]+=D[i+1];
    // FOR_(i,0,n) printf("%lld ",L[i]+D[i+1]); puts("<- L");
    // FOR_(i,0,n) printf("%lld ",R[i]+D[i+1]); puts("<- R");
    vector<pll> vec;
    FOR_(i,1,n) vec.push_back({L[i]+D[i+1],R[i]+D[i+1]});
    sort(vec.begin(),vec.end());
    // reverse(vec.begin(),vec.end());
    ll xlim=L[0]+D[1],ylim=R[0]+D[1];
    ll res=0;
    for (auto [x,y]:vec) {
        // printf("%lld %lld\n",xlim,ylim);
        max_(res,min(xlim,x)+ylim);
        min_(ylim,y);
    }
    max_(res,xlim+ylim);
    printf("%lld\n",res);
