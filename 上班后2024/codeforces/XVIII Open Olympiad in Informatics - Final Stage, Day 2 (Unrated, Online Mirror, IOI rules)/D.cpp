...
Info operator+(const Info &a,const Info &b) {
    ...
    c.maxa=a.maxa;
    c.minb=b.minb;
    if (a.prefmin==a.sum+b.prefmin) {
        max_(c.res,b.maxa-a.minb);
    } else if (a.prefmin<a.sum+b.prefmin) {
        min_(c.minb,a.minb);  // 左边多出来负数
    } else {
        max_(c.maxa,b.maxa);
    }
    return c;
...
int solve() {
    ...
    vector<pii> vec;
    FOR_(i,1,n) {
        vec.push_back({A[i],i+n});
        vec.push_back({B[i],i});
    }
    sort(vec.begin(),vec.end());
    vector<int> id(n*2+1);
    REP_(k,n*2) id[vec[k].second]=k;
    SegmentTree<Info> seg(n*2);
    ll all=0;
    FOR_(k,1,n) {
        all+=A[k]-B[k];
        seg.modify(id[k],{1,0,0,-INF,B[k]});
        seg.modify(id[k+n],{-1,-1,0,A[k],INF});
        ll mor=seg.rangeQuery(0,n*2).res;
