...
int solve() {
    ...
    // FOR_(x,0,n) FOR_(i,0,k) ksum[x]+=mint(i).pow(x)*c[i];
    vector<mint> pw=c;
    FOR_(x,0,n) FOR_(i,0,k) ksum[x]+=pw[i],pw[i]*=i;
    // FOR_(i,0,k) cout<<c[i]<<" "; puts("<- ksum");

    vector<mint> f(n+1);
    rFOR_(i,0,n) {
        rFOR_(j,i,n) {
            mint base=ksum[j-i]*Comb(j,i);  // 分治fft
            if (j>i) g[i]-=f[j]*base;
            else f[i]=g[i]/base;
        }
    }
    FOR_(i,0,n) cout<<f[i].x<<" ";
    cout<<"\n";
    return 0;
}
int main() {
