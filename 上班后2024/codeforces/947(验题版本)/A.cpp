...
int solve() {
    int n;
    scanf("%d", &n);
    vector<int> A(n+1),f(n+n+1);
    A[0]=INF;
    FOR_(i,1,n) scanf("%d",&A[i]),A.push_back(A[i]);
    FOR_(i,1,n+n) {
        if (A[i]>=A[i-1]) f[i]=f[i-1]+1;
        else f[i]=1;
    }
    // FOR_(i,1,n+n) printf("%d ",A[i]); puts("<- A");
    // FOR_(i,1,n+n) printf("%d ",f[i]); puts("<- f");
    FOR_(i,1,n+n) if (f[i]>=n) return 0*puts("Yes");
    puts("No");
    return 0;
}
int main() {
