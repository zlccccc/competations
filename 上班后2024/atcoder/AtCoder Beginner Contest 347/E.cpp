...
int solve() {
    ...
    FOR_(i,1,n) {
        ...
        pref[i]=pref[i-1]+S.size();
    }
    // FOR_(i,1,n) printf("%d ",pref[i]); puts("<- pref");
    vector<ll> res(m+1);
    S.clear();
    FOR_(i,1,n) {
        int x=A[i];
        if (S.count(x)) S.erase(x),res[x]+=pref[i-1];
        else S.insert(x),res[x]-=pref[i-1];
    }
    for (int x:S) res[x]+=pref[n];
    FOR_(i,1,m) printf("%lld ",res[i]); puts("");
    return 0;
}
int main() {
