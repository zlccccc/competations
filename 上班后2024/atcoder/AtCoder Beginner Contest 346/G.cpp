...
void update(int x,int l,int r,int val,int L,int R) {
    if (l<=L&&R<=r) {
        T[x]+=val;
        if (!T[x]) all[x]=all[x<<1]+all[x<<1|1];
        else all[x]=R-L+1;
        return;
    }
    int mid=(L+R)/2;
...
int solve() {
    ...
    vector<vector<ar3>> upd(n+2);
    for (auto [item,vec]:MP) {
        int pre=0;
        vec.push_back(n+1);
        for (int i=0;i<vec.size()-1;i++) {
            int v=vec[i],nxt=vec[i+1];
            // printf("%d %d %d %d\n",pre+1,v,v,nxt-1);
            upd[v].push_back({pre+1,v,1});
            upd[nxt].push_back({pre+1,v,-1});
            pre=v;
        }
    }
    ll res=0;
