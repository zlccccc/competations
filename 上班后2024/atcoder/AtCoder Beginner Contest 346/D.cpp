...
int solve() {
    ...
    arr[0][0][0]=arr[0][1][0]=0;
    FOR_(i,1,n) {
        REP_(k,2) {
            int mor=s[i]-'0'==k?0:A[i];
            arr[i][k][0]=arr[i-1][k^1][0]+mor;
            arr[i][k][1]=arr[i-1][k^1][1]+mor;
            if (i>1) arr[i][k][1]=min(arr[i-1][k][0],arr[i-1][k^1][1])+mor;
            // printf("%d %d\n",arr[i][k][0],arr[i][k][1]);
        }
    }
    printf("%lld\n",min(arr[n][0][1],arr[n][1][1]));
    return 0;
}
int main() {
