...
void update(int x,int l,int r,int val,int L,int R) {
    if (l<=L&&R<=r) {
        if (val<0) S[x].erase(S[x].lower_bound(-val));
        else S[x].insert(val);
        return;
    }
    int mid=(L+R)/2;
...
int query(int x,int pos,int L,int R) {
    int cur=0;
    if (S[x].size()) cur=*S[x].rbegin();
    if (L==R) return cur;
    int mid=(L+R)/2;
    if (pos<=mid) return max(cur,query(x<<1,pos,L,mid));
    if (mid<pos) return max(cur,query(x<<1|1,pos,mid+1,R));
}
int A[maxn];
int solve() {
    int n;
    scanf("%d",&n);
    FOR_(i,1,n) scanf("%d",&A[i]),update(1,i,i,A[i],1,n);
    int q;
    scanf("%d",&q);
    vector<ar3> vec(q+1);
    FOR_(i,1,q) {
        ...
        if (op==1) {
            ...
            update(1,vec[i][0],vec[i][1],vec[i][2],1,n);
        }
        if (op==2) {
            int x;
            scanf("%d",&x);
            update(1,vec[x][0],vec[x][1],-vec[x][2],1,n);
        }
        if (op==3) {
            int x;
            scanf("%d",&x);
