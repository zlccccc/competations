...
bool mark[maxn];
deque<int> dfs2(int x,int fa) {
    deque<int> ret={x};
    for (int v:edge[x]) {
        if (v==fa) continue;
        auto sub=dfs2(v,x);
        if (mark[v]) merge(ret,sub);
        else merge(sub,ret),ret.swap(sub);
    }
    return ret;
}
...
int solve() {
    ...
    dfs(1,0);
    FOR_(i,2,n) id[i]=i;
    sort(id+2,id+1+n,[](int x,int y) {
        return sz[x]>sz[y];
    });
    FOR_(i,2,n) {
        int x=id[i];
        if (k>=sz[x]) k-=sz[x],mark[x]=1;  // smaller
    }
    // FOR_(i,1,n) printf("%d ",mark[i]); puts(" <- mark");
    if (k) return 0*puts("No");
    auto vec=dfs2(1,0);
    puts("Yes");
    int tot=0;
    for (int v:vec) res[v]=++tot;
