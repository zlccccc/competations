...
int solve() {
    ...
    string s;
    cin>>s;
    auto getres=[&](vector<int> lim,char c) {
        ...
        FOR_(i,1,n) {
            ...
            assert(1<=x&&x<=n);
            assert(!mark[x]);
            // printf("x=%d; lim=%d; res=%lld\n",x,lim[x],res.x);
            if (mark[lim[x]]) {
                if (s[x-1]=='?') res=res*2;
            } else {
                if (s[x-1]!='?'&&s[x-1]!=c) res=0;
            }
            // printf("%d  %lld\n",x,res.x);
            mark[x]=1;
        }
    ...
    // printf("%d %d\n",couldleft,couldright);
    mint res=0;
    {
        vector<int> lim(n+1);
        FOR_(i,1,n-1) lim[i]=i+1; lim[n]=1;
        res+=getres(lim,'L');
    }
    {
        vector<int> lim(n+1);
        FOR_(i,2,n) lim[i]=i-1; lim[1]=n;
        res+=getres(lim,'R');
    }
    cout<<res.x;
    return 0;
