...
int solve() {
    ...
    set<int> mask;
    REP_(i,n) mask.insert(1<<i);
    REP_(i,n) REP_(j,n) {
        double p=X[j]/X[i];
        double a=(p*Y[i]-Y[j])/(p*X[i]*X[i]-X[j]*X[j]);
        double b=(Y[i]-a*X[i]*X[i])/X[i];
        if (a>=0||abs(X[i]-X[j])<eps) continue;
        int cur=0;
        REP_(k,n) if (abs(Y[k]-a*X[k]*X[k]-b*X[k])<eps) cur|=1<<k;
        // printf("cur=%d\n",cur);
        mask.insert(cur);
    }
    ...
    res[0]=0;
    REP_(sta,1<<n) {
        for (auto x:mask) min_(res[sta|x],res[sta]+1);
    }
    printf("%d\n",res.back());
    return 0;
}
