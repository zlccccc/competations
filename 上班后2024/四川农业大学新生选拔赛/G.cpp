...
int solve() {
    ...
    FOR_(i,1,n) {
        ...
        if (cur=="start") {
            ...
            string s;
            cin>>s;
            auto it=rS.lower_bound({s,0});
            if (it!=rS.end()&&it->first==s) cout<<"Error\n";
            else {
                cout<<"OK\n";
                t++;
                S.insert({t,s});
