...
int A[maxn];
vector<pair<int,pii> > v;//gcd;left;haveval
int main() {
	int n,i,cnt=0;
	pair<int,int> ans;
	read(n);
	...
	FOR(i,1,n) {//不想改啊...
		...
		for (auto p:v) {
			int g=gcd(A[i],p.first);
			int l=p.second.first,r=p.second.second;
			if (g!=p.first) r=0; r|=(g==A[i]);
			if (!nxt.count(g)||nxt[g]>make_pair(l,r))
				nxt[g]=make_pair(l,r);
			if (r) {
				int len=i-l+1;
				if (len>ans.first) ans.first=len,ans.second=0;
				if (len==ans.first) ans.second++;
			}
		} v.clear();
		for (auto now:nxt) v.push_back(now);
		cnt++;//use this
		// for (auto now:nxt) printf("%d-%d %d %d\n",now.second.first,i,now.first,now.second.second);
	}
	printf("%d\n",ans.second);
}
/*
