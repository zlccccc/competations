...
// answer: prev|(last)|prev|(last)|prev|(last)|prev
char s[maxn];
vector<int> p;
vector<int> dp[17];
int main() {
    ...
    scanf("%s",s+1);
    n=strlen(s+1); //position
    FOR(i,1,n+1) {
        if (i==n+1||s[i]!=s[i-1]) p.push_back(i);
        if (i!=1&&s[i]<s[i-1]) break;
    }
    dp[p.size()-1].resize(n+2);
    dp[p.size()-1][*p.rbegin()]=1; // to-last

    // for (int v:p) printf("%d ",v); puts("<- position");
    rREP(k,p.size()-1) {
        int l=p[k+1]-p[k]-1;
        dp[k].resize(n+2);
        FOR(i,p[k],p[k+1]-1) dp[k][i]=1;
        // // 
        // // printf("k=%d; p[k]=%d; c=%c l=%d\n",k,p[k],s[p[k]],l);
        // FOR(i,p[k+1],n+1) { // dp[k][i]: p[k]->i all chosen方案数
        //     FOR(j,0,i) {
        //         dp[k][i]+=dp[k+1][j]*C(i-j+l,l);
        //         // printf("%d->%d: %d * %lld(C %d %d)\n",j,i,dp[k+1][j],C(i-j+l,l),i-j+l,l);
        //         // printf("%d->%d: %d * %lld\n",j,i,dp[k+1][j],C(i-j+l,l));
        //     }
        //     if (i<=n&&s[i]<=s[p[k]]) break;
        // }

        vector<int> multi(n+2);
        REP(i,n+1) multi[i]=C(l+i,i);
        poly newval=poly(dp[k+1])*poly(multi);
        FOR(i,p[k+1],n+1) {
            dp[k][i]=newval[i];
            if (i<=n&&s[i]<=s[p[k]]) break;
        }
        // FOR(i,0,n+1) printf("%d ",dp[k][i]); puts("<- dp");
    }
    printf("%d\n",dp[0][n+1]);
}
/*
