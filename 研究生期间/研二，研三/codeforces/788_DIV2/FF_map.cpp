...
LL calc(ll x,ll k) {
    ...
    if (k==-1) return 1;
    // printf("%lld %lld\n",x/n,1ll<<(k+1));
    // printf("x=%lld; k=%lld; %lld %lld;\n",x,k,x/n,1ll<<(k+1));
    if (x/n>(1ll<<(k+1))) return powMM(2,(k+1)*(n-1));
    if (MP[k].count(x)) return MP[k][x];
    ll ret=0,i;
    FOR(i,0,n) {
        // printf("i=%lld\n",i);
        if (x-(i<<k)<0) break;
        if ((i&1)!=value[k]) continue;
        add_(ret,C(n,i)*calc(x-(i<<k),k-1)%M);
    }
    // if (ret) printf("ret=%lld; x=%lld; k=%lld;\n",ret,x,k);
    return MP[k][x]=ret;
}
int main() {
    ll l,r,z,i;
