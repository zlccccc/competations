...
int main() {
    ...
    FOR(i,1,n*2) if (!mark[i]) {
        ...
        int st,k;
        REP(st,noww.size()) {
            int p=noww[(st+noww.size()-1)%noww.size()];//does not contain this
            // printf("p=%d\n",p);
            // int lx=px-1,rx=px+1,ly=py-1,ry=py+1;
            int lx=n,rx=1,ly=n*2,ry=n+1;
            FOR(k,0,noww.size()-1) {//(k+1)/2: okay;
                ...
                if (k&1) {
                    int baseans=ans;
                    // ans+=(ll)(ly-n)*(n*2-ry+1)*(n-rx+1)*lx;
                    //[px,py] not inside(remove)
                    if (p<=n&&p<=lx) ans+=(ll)(ly-n)*(n*2-ry+1)*(n-rx+1)*(lx-p);
                    if (p<=n&&rx<=p) ans+=(ll)(ly-n)*(n*2-ry+1)*(p-rx)*lx;
                    if (p>n&&p<=ly) ans+=(ll)(ly-p)*(n*2-ry+1)*(n-rx+1)*lx;
                    if (p>n&&ry<=p) ans+=(ll)(ly-n)*(p-ry)*(n-rx+1)*lx;
                    // if (k&1) printf("%d(%d,%d;%d,%d;+%d) ",nowval,lx,rx,ly,ry,ans-baseans);
                }
            }
            // printf("   nowans=%lld\n",ans);
            // puts("  <- nowval");
