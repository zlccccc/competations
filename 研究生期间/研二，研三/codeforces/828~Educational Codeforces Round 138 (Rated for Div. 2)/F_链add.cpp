...
void update(int x,int y,int d,int val) {
    while (top[x]!=top[y]) {
        ...
        T[d].update(id[top[x]],id[x],val);
        x=fa[top[x]];
    }
    if (dep[x]>dep[y]) swap(x,y);
    if (son[x]) T[d].update(id[son[x]],id[y],val); // x is lca
    // more
    while (d) {
        T[d].update(id[x],id[x],val);
        T[d-1].update(id[x],id[x],val);
        d--; x=fa[x];
    }
    T[d].update(id[x],id[x],val);
}
