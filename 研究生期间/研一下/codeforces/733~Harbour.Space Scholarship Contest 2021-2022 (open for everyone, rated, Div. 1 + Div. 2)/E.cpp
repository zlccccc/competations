...
int main() {
    ...
    FOR(_,1,T){
        int i;
        int n,m;
        MP.clear(); ans.clear(); check.clear();
        read(n); read(m);
        FOR(i,1,n) read(A[i]);
        FOR(i,1,n) MP[(n-A[i]+i)%n]++;
        for (auto now:MP) if (now.second>=n-m*2) check.push_back(now.first);
        for (int value:check) {
            // printf("check %d %d  %d\n",now.first,now.second,n-m*2);
            FOR(i,1,n) B[i]=(A[i]+value-1+n)%n+1;
            FOR(i,1,n) mark[i]=0;
            // FOR(i,1,n) printf("%d ",B[i]); puts("<- B");
            int nowcount=0;
            FOR(i,1,n) if (B[i]!=i) nowcount++;
            FOR(i,1,n) if (!mark[i]&&B[i]!=i) {
                for (int k=i;!mark[k];k=B[k]) mark[k]=1;
                // printf("i,k=%d %d\n",i,k);
                nowcount--;
            } if (nowcount<=m) ans.push_back(value);
            // printf("%d: use %d\n",now.first,nowcount);
        }
        // printf("%d",(int)ans.size());
        print(ans.size());
        sort(ans.begin(), ans.end());
        for (int v:ans) print(' '),print(v);
        // printf(" %d",v);
        println("");
    }
}
