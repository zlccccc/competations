...
int A[maxn];
ll f[maxn],g[maxn],fpre[maxn],gpre[maxn];//f+g*x
vector<int> noww;
int main() {
    int i,j;
    int n,m;//A[i]%A[j]=A[i]-A[i]/A[j]*A[j];
    // scanf("%d",&n);
    read(n);
    ...
    FOR(_,1,n) {
        if (noww.size()>SIZE) {
        // if (noww.size()) {
            FOR(i,0,300000) fpre[i]=gpre[i]=0;
            for (int v:noww) {
                // if (v<SZ2) {
                //     FOR(j,1,300000) f[j]+=v%j+j%v;
                // } else {
                // x%v
                for (int k=v;k<=300000;k+=v) fpre[k]-=v;
                gpre[0]+=1;
                // v%x
                for (int l=1;l<=v;) {//v%x=0: v-v/x*x
                    int k=v/l,r=v/k+1;
                    fpre[l]+=v; fpre[r]-=v;
                    gpre[l]-=k; gpre[r]+=k;
                    // printf("l,r=%d %d\n",l,r);
            ...
            FOR(i,1,300000) fpre[i]+=fpre[i-1],f[i]+=fpre[i];
            FOR(i,1,300000) gpre[i]+=gpre[i-1],g[i]+=gpre[i];
            noww.clear();
        }
        // FOR(i,1,10) printf("%lld ",f[i]+g[i]*i); puts("<- f"); 
        ll value=A[_];
        ans+=f[value]+g[value]*value;
        // for (int v:noww) ans+=v%value+value%v;
        noww.push_back(value);
        // print(ans); print(" ");
    }
    printf("time=%d\n", clock()-start);
}
