...
int main() {
    ...
    // solve(6,7);
    // solve(7,7);
    FOR(i,1,400) {//not unique
        f[i][0]=1;
        FOR(j,1,200) f[i][j]=(f[i-1][j-1]*(i-1)+f[i-1][j])%M;
    }
    // FOR(i,1,10) {FOR(j,1,10) printf("%-10lld ",f[i][j]); printf(" << %d\n",i);}
    FOR(i,1,400) FOR(j,1,200) {//number unique
        // FOR(k,0,i) {
        //     if (!(k&1)) add_(g[i][j],C(i,k)*f[i-k][j]%M);
        //     else add_(g[i][j],M-C(i,k)*f[i-k][j]%M);
        // }
        g[i][j]=f[i][j];
        FOR(k,1,i) add_(g[i][j],M-C(i,k)*g[i-k][j]%M);
    }
    // FOR(i,1,10) {FOR(j,1,10) printf("%-10lld ",g[i][j]); puts("");}
    scanf("%d%d",&n,&k);
    ans[0]=1;//not change
    FOR(j,1,k) {
