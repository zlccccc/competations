          ...
          for (int m1 = 0; m1 < 16; ++m1) {
            ...
            if (area == (b0 + b1 + b2) % 4) {
              if (m0 == m1 && b0 == b1) {
                ...
                more=cnt[b0][m0] * (cnt[b0][m0] - 1);
              } else {
                res[b0 & 1] += cnt[b0][m0] * cnt[b1][m1];
                more=cnt[b0][m0] * cnt[b0][m0];
              }
              int g0=b0,g1=b1,g2=b2;
              int x2=X[k],y2=Y[k];
              int x0=((m0 >> 2)+x2)&3,y0=((m0 & 3)+y2)&3;
              int x1=((m1 >> 2)+x2)&3,y1=((m1 & 3)+y2)&3;

              int siz=area;
              if (more) printf("i=%d: (%d,%d,%d), (%d,%d,%d), (%d,%d,%d); siz=%d %d (%d)\n",k,g0,x0,y0,g1,x1,y1,g2,x2,y2,siz,more);
            }
          }
        }
      }
