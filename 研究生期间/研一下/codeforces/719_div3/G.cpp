...
int main() {
    ...
    FOR(i,1,n) FOR(j,1,m) read(id[i][j]); //scanf("%d",&id[i][j]);
    // FOR(i,1,n) FOR(j,1,m) printf("%d%c",id[i][j]," \n"[j==m]);
    Q.push(make_pair(1,1)); dis[1][1]=0;
    while (Q.size()) {
        auto now=Q.front(); Q.pop();
        int i=now.first,j=now.second,k;
        REP(k,4) {
            int x=ax[k]+i,y=ay[k]+j;
            if (id[x][y]>=0&&dis[x][y]==INFF) {
                dis[x][y]=dis[i][j]+w;
                Q.push(make_pair(x,y));
            }
        } if (id[i][j]>0) MX=min(MX,id[i][j]+dis[i][j]);
    } ll ans=dis[n][m];
    // FOR(i,1,n) FOR(j,1,m) printf("%-8d%c",dis[i][j]," \n"[j==m]);
 
    memset(dis,0x3f,sizeof(dis));
    Q.push(make_pair(n,m)); dis[n][m]=0;
    while (Q.size()) {
        auto now=Q.front(); Q.pop();
        int i=now.first,j=now.second,k;
        REP(k,4) {
        ...
        } if (id[i][j]>0) MY=min(MY,id[i][j]+dis[i][j]);
    } ans=min(ans,MX+MY);
