...
int main() {
    ...
    FOR(k,1,n) {
        if (A[k]!=-1) {
            ...
            rFOR(i,k+1,n) {
                if (A[i]==-1) {
                    rFOR(j,1,n-i+1) add_(vr[j],vr[j-1]);
                } else if (A[i]<A[k]) {
                    add_(vr[0],vr[0]); FOR(j,0,n-i+1) add_(vr[j],vr[j+1]);
                } else FOR(j,0,n-i+1) vr[j]=vr[j]*2%M;
            }
            FOR(i,1,n) add_(vr[i],vr[i-1]);
            FOR(i,0,n) add_(ans,(ll)vl[i]*vr[i]%M*A[k]%M);
            // printf("k=%d; ans=%d\n",k,ans);
